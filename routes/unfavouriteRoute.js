...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (!fsArrayInclude(userFav, req.body.route)) {
      throw new Error("unsuccessful: route not saved as favourite")
    }
  ...
  try {
    ...
    res.status(400).send("unsucessful: unable to remove from favourites");
    return;
  }

  try {
    await routeRef.update({ FavouritedUsers: FieldValue.arrayRemove(req.body.user) })
    res.status(200).send("successful: route removed from favourites");
  } catch (error) {
